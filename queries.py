...
def Q_2(conn, execution_time):
    ...
    query = """ SELECT 
    Players.player_name,
    COUNT (Events.event_id) AS shots
FROM 
    Players
JOIN Events ON Players.player_id = Events.player_id
...
AND Event_types.type_name = 'Shot'
GROUP BY 
    Players.player_id, Players.player_name
ORDER BY shots DESC;"""
...
WHERE 
    Players.team_id IN (
        ...
        WHERE
            Competitions.competition_name = 'La Liga' AND
            Seasons.season_name IN ('2020/2021', '2019/2020', '2018/2019')
        UNION
        SELECT DISTINCT
            Matches.away_team_id
        FROM
...
WHERE 
    Teams.team_id IN (
        ...
        WHERE
            Competitions.competition_name = 'La Liga' AND
            Seasons.season_name IN ('2020/2021')
        UNION
        SELECT DISTINCT
            Matches.away_team_id
        FROM
...
AND Event_types.type_name = 'Pass'
GROUP BY 
    Teams.team_name
ORDER BY pass DESC;"""
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT 
    Players.player_name,
    COUNT (Events.event_id) AS receives
FROM 
    Players
JOIN Events ON Players.player_id = Events.player_id
JOIN Event_types ON Events.type_id = Event_types.type_id
...
AND Event_types.type_name = 'Ball Receipt*'
GROUP BY 
    Players.player_name
...
def Q_7(conn, execution_time):
    ...
    query = """SELECT 
    Players.player_name,
    COUNT (Events.event_id) AS Goals
FROM 
    Players
JOIN Events ON Players.player_id = Events.player_id
WHERE 
...
def Q_9(conn, execution_time):
    ...
    query = """ SELECT 
    Players.player_name,
    COUNT (Events.event_id) AS dribble
FROM 
    Players
JOIN Events ON Players.player_id = Events.player_id
WHERE 
...
AND Events.dribbled_outcome = 'Complete'
GROUP BY 
    Players.player_name
ORDER BY dribble DESC;
...
def Q_10(conn, execution_time):
    ...
    query = """ SELECT 
    Players.player_name,
    COUNT (Events.event_id) AS Dribble_pass
FROM 
    Events
...
AND Event_types.type_name = 'Dribbled Past'
GROUP BY 
    Players.player_name
ORDER BY Dribble_pass ASC;
"""
